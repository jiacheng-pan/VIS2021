...
\begin{compactenum}[\textbf{Condition} 1.]
    \item Two nodes share a same value of a same attribute, e.g., linking two movies with the same director.
...
At last, we textualize the linking condition of the graph by filling templates. 

``Two nodes are connected if their attributes {\color{blue} [attribute]} are with a same value\{ ({\color{green}[value]})?\}.''

``Two nodes are connected if their attributes {\color{blue} [attribute]} share an intersection\{ ({\color{green}[value]})?\}.''

``Two nodes are connected if their attributes {\color{blue} [attribute]} are significantly close\{ to {\color{green}[value]})?\}.''
